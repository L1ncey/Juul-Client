...
import juul.inject.wrappers.DynamicTextureWrapper;

public class MinecraftFontRenderer extends CFont implements Utils {
    ...
    protected DynamicTextureWrapper texBold;
    protected DynamicTextureWrapper texItalic;
    protected DynamicTextureWrapper texItalicBold;
    ...
    public int drawString(String text, double x2, float y2, int color) {
        return (int) this.drawString(text, x2, y2, color, false, 8.8f);
    }
    ...
    public int drawNoBSString(String text, double x2, float y2, int color) {
        return (int) this.drawNoBSString(text, x2, y2, color, false, 8.8f);
    }
    ...
    public float drawNoBSString(String text, double x2, double y2, int color, boolean shadow, float kerning) {
        x2 -= 1.0;
        if (text == null) {
            return 0.0f;
        }
        ...
        while (i2 < size) {
            ...
            if (character == '\u00a7' && i2 < size) {
                ...
                GL11.glEnd();
                if (strikethrough) {
                    this.drawLine(x2, y2 + (double) (currentData[character].height / 2), x2 + (double) currentData[character].width - kerning, y2 + (double) (currentData[character].height / 2), 1.0f);
                }
                if (underline) {
                    this.drawLine(x2, y2 + (double) currentData[character].height - 2.0, x2 + (double) currentData[character].width - kerning, y2 + (double) currentData[character].height - 2.0, 1.0f);
                }
                x2 += (double) (currentData[character].width - kerning + this.charOffset);
            }
            ++i2;
    ...
    public int getHeight() {
        return (this.fontHeight - (int) (8)) / (int) (2);
    }
    ...
    public List<String> wrapWords(String text, double width) {
        ...
        if ((double) this.getStringWidth(text, 8.8f) > width) {
            ...
            while (j2 < length) {
                ...
                if ((double) this.getStringWidth(String.valueOf(String.valueOf(String.valueOf(currentWord))) + word + " ", 8.8f) < width) {
            ...
            if (currentWord.length() > 0) {
                if ((double) this.getStringWidth(currentWord, 8.8f) < width) {
    ...
    private float getCharWidthFloat(char p_78263_1_) {
        //CleanClient.addChatMessage(((charData[p_78263_1_].width/2)-4)+"");
        if (p_78263_1_ == 167) {
            return -1.0F;
